...
export default function HabitModal({ onClose }: { onClose: () => void }) {
  return (
    <Modal title="Nuevo hábito" onClose={onClose}>
      <HabitForm mode="create" onSuccess={onClose} />
    </Modal>
  )
}

export function HabitNewButton({ variant = 'header' }: { variant?: 'header' | 'sidebar' | 'fab' }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className={cls}>
        ...
        {variant !== 'fab' && (variant === 'header' ? 'Nuevo' : 'Nuevo hábito')}
      </button>
      {open && <HabitModal onClose={() => setOpen(false)} />}
    </>
  )
}
